...
public class Index extends Node
{
	...
	protected Node chooseSubtree(Region mbr, int level, Stack<Integer> pathBuffer)
	{
		...
		return n.chooseSubtree(mbr, level, pathBuffer);
	}
	...
	protected Node[] split(byte[] pData, Region mbr, int id)
	{
		...
		for(int i: g1) left.insertEntry(null, m_pMBR[i], m_pIdentifier[i]);
		for(int i: g2) right.insertEntry(null, m_pMBR[i], m_pIdentifier[i]);
	...
	{
		public int compare(OverlapEntry e1, OverlapEntry e2)
		{
